...
function DnsDetails({ investigation: i }: { investigation: Investigation }) {
  ...
  return <><p className="detail-intro">DNS is the address book, not the journey. These answers were returned by {i.dns.resolver}. They can vary by resolver, location, and time.</p>
...
export function RedirectDetails({ investigation: i }: { investigation: Investigation }) {
  if (!i.http) return <UnknownLayer message={i.providers.http.message ?? "The HTTP response hasn't arrived yet."} />;
...
export function NetworkDetails({ investigation: i }: { investigation: Investigation }) {
  ...
  return <><p className="detail-intro">An IP is a network address, not a pin on a map. An autonomous system (ASN) identifies a network announcing routes, not necessarily the website’s origin host.</p>
    ...
    <div className="network-addresses">{addresses.map((address) => <details className="network-address" key={address.ip} open={addresses.length === 1}><summary><span className="record-type">IPv{address.version}</span><code>{address.ip}</code><ArrowUpRight size={15} /></summary><DefinitionList items={[
      { label: "ASN", value: address.asn ?? "Unknown" },
      { label: "Organization", value: address.organization ?? "Unknown", help: "The organization reported by the network-data source. Origin hosting remains unknown." },
    ...
    ]} /><div className="evidence-provenance"><span>{address.source}</span>{address.sourceUrl && <SourceLink url={address.sourceUrl} />}</div></details>)}</div>
...
function TechnologyDetails({ investigation: i, infrastructure = false }: { investigation: Investigation; infrastructure?: boolean }) {
  ...
  return <><p className="detail-intro">{infrastructure ? "Edge and hosting hints are interpretations. Expand each one to see the exact signals behind it. A network affiliation never proves where an origin is hosted." : "Technologies leave traces in response headers and HTML. We report those traces, without running JavaScript or guessing the rest of the stack."}</p>
...
export function HistoryEmpty() {
  return <div className="history-empty"><Clock3 size={25} strokeWidth={1.1} /><div><span className="eyebrow">TIME IS ANOTHER LAYER</span><h3>No historical observations yet.</h3><p>This version does not save investigations or query a history provider. Export an observation to keep your own record. We won’t invent a past for this hostname.</p></div></div>;
}
...
export function LayerDrawer({ investigation, layer, onClose, onLayerChange }: { investigation: Investigation; layer: Layer | null; onClose: () => void; onLayerChange: (layer: Layer) => void }) {
  return <Dialog open={!!layer} onClose={onClose} title={layer ? layerNames[layer] : "Inspect a layer"} eyebrow="BENEATH THE SURFACE" wide>
    ...
    {layer && <div key={layer} className="layer-content"><LayerContent investigation={investigation} layer={layer} />
