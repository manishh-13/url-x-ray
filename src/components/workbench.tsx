...
function Story({ findings, onSelect, running }: { findings: Finding[]; onSelect: (layer: Layer) => void; running: boolean }) {
  const reduced = useReducedMotion();
  return <aside className="story-panel" aria-label="The story behind this URL">
    <div className="story-header"><span className="eyebrow">THE STORY SO FAR</span><Waypoints size={16} strokeWidth={1.2} /></div>
    <h2>A little less invisible.</h2><p className="story-intro">Follow the evidence, one layer at a time.</p>
    <div className="story-findings"><AnimatePresence initial={false}>{findings.map((finding, i) => <motion.article key={finding.id} initial={reduced ? false : { opacity: 0, y: 9 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} transition={{ duration: .45 }} className={`story-finding finding-${finding.confidence}`}>
      <span className="story-step-number">{String(i + 1).padStart(2, "0")}</span><div><ConfidenceBadge confidence={finding.confidence} compact /><h3>{finding.title}</h3><p>{finding.description}</p><button onClick={() => onSelect(finding.layer)} className="story-inspect">{finding.confidence === "unknown" ? "What’s missing" : "Show the evidence"}<ArrowUpRight size={12} /></button></div>
    ...
    <div className="story-bottom"><span className="evidence-mark">∴</span><p>Every statement has a source.<br />Every inference is labeled.</p></div>
  </aside>;
}
...
export function Workbench({ investigation: i, running, error, onRun, onCancel, onHome, onMethodology }: {
  ...
  return <main id="main-content" className="workbench">
    ...
    <div className="depth-content">
      ...
      </div><Story findings={depth === 1 ? findings.filter((finding) => finding.layer !== "history") : findings} onSelect={setLayer} running={running} /></div>}
    ...
    <div className="observation-scope"><span>THE LIMITS ARE PART OF THE PICTURE.</span><p>This is one observation from this server. DNS answers may vary. Edge networks can conceal origins. Headers can be changed. Unknown means unknown.</p></div>
