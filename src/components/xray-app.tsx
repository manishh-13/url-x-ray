...
import { IS_BROWSER_EDITION, REPOSITORY_URL, readSharedHostname } from "@/lib/edition";
...
export function XRayApp({ initialHostname }: { initialHostname?: string }) {
  ...
  return <MotionConfig reducedMotion="user"><div className={`app-shell ${investigation ? "has-investigation" : ""}`}>
    ...
    <footer className="site-footer"><div><XRayMark /><span>Every URL has a story.</span></div><p>Made for curious minds.</p><div className="footer-links"><a className="project-source" href={REPOSITORY_URL} target="_blank" rel="noopener noreferrer">GitHub source<ArrowUpRight size={14} aria-hidden="true" /></a><button onClick={() => setPrivacy(true)}>Privacy & scope<ArrowUpRight size={12} /></button></div></footer>
